...
12. [Solução de problemas comuns](#️-solução-de-problemas-comuns)
13. [Comandos úteis](#-comandos-úteis)
14. [Arquivos padrão do Laravel](#-arquivos-padrão-do-laravel-não-customizados)
...
|------------|--------------|----------------|----------|
| **PHP** | 8.5+ | Linguagem que executa o Laravel | [windows.php.net/download](https://windows.php.net/download/) |
| **Composer** | 2.x | Gerenciador de pacotes do PHP | [getcomposer.org](https://getcomposer.org/download/) |
| **MySQL** | 8.0+ | Banco de dados relacional | [dev.mysql.com/downloads/installer](https://dev.mysql.com/downloads/installer/) |
| **Node.js** | 18+ | Necessário para alguns pacotes do Laravel | [nodejs.org](https://nodejs.org/en/download/) |
| **Git** | 2.x+ | Clonar o repositório | [git-scm.com/downloads](https://git-scm.com/downloads) |

> 💡 **Sobre o PHP:** O Laravel já vem incluído como dependência do projeto — você **não precisa instalar o Laravel separadamente**. Basta ter o PHP instalado e rodar `composer install` que o Laravel é baixado automaticamente.

> ⚠️ **Sobre o MySQL:** O instalador do MySQL 8.0 para Windows **pede para criar uma senha** durante a instalação. Guarde essa senha — você vai precisar dela no `.env` e para acessar o MySQL pelo CMD.
...
```bash
php --version
# Esperado: PHP 8.5.x ou superior
...
# incluindo o próprio Laravel
# (pode demorar alguns minutos na primeira vez)
composer install
```

> 💡 **Por que isso é necessário?** O arquivo `composer.json` lista todas as bibliotecas que o projeto usa (como o Sanctum, Laravel, etc.). O `composer install` lê essa lista e baixa tudo para a pasta `vendor/`. Essa pasta não vai para o GitHub — cada desenvolvedor precisa rodar esse comando localmente.
...
# ⚠️ Mude DB_CONNECTION de sqlite para mysql e configure as linhas abaixo:
DB_CONNECTION=mysql
DB_HOST=127.0.0.1
DB_PORT=3306
...
> ⚠️ O arquivo `.env` **nunca deve ser enviado para o GitHub** pois contém informações sensíveis.

> 💡 **Senha do MySQL:** Se você instalou o MySQL 8.0 pelo instalador oficial, foi pedida uma senha durante a instalação. Coloque essa senha em `DB_PASSWORD=sua_senha`. Se não criou senha, deixe vazio.

### Passo 4 — Criar o banco de dados no MySQL

Abra o CMD e acesse o MySQL:

```bash
# Se instalou sem senha:
mysql -u root

# Se instalou com senha (MySQL 8.0 padrão):
mysql -u root -p
# Vai pedir a senha que você criou durante a instalação
```
...
> 💡 **O que é MariaDB?** O instalador do MySQL para Windows às vezes instala o MariaDB — que é um banco de dados 100% compatível com MySQL. Os comandos e configurações são exatamente os mesmos. Se aparecer `MariaDB` no terminal, não se preocupe — funciona igual.
...
> 💡 **O que são seeders?** São arquivos que inserem dados iniciais no banco. O `UserSeeder` cria 3 equipes, 10 gestores, 10 colaboradores e 2 usuários fixos para teste. O `ClientSeeder` cria 10 clientes fictícios de uma agência de viagens.
...
> ⚠️ **IMPORTANTE:** Ainda é necessário iniciar o frontend (Next.js) em outro terminal usando `npm run dev`. Mais informações em: `desafio-rootcode-frontend/README.md`
...
### Usuários fixos para avaliação

```
┌───────────────────────────────────────────────────┐
│  Perfil      │ Email                   │ Senha    │
│───────────────────────────────────────────────────│
│  Gestor      │ gestor@rootcode.com     │ password │
│  Colaborador │ colaborador@rootcode.com│ password │
└───────────────────────────────────────────────────┘
```

### Usuários gerados automaticamente

O `UserSeeder` também cria **3 equipes**, **10 gestores** e **10 colaboradores** com dados aleatórios. A senha de todos é `password`.
...
### `mysql` não é reconhecido no CMD

O MySQL foi instalado mas não está no PATH do Windows. Solução:

1. Pesquise **"variáveis de ambiente"** no menu Iniciar
2. Clique em **"Editar as variáveis de ambiente do sistema"**
3. Clique em **"Variáveis de Ambiente"**
4. Em **"Variáveis do sistema"**, clique em **Path** → **Editar**
5. Clique em **Novo** e adicione:
```
C:\Program Files\MySQL\MySQL Server 8.0\bin
```
6. Clique OK em tudo, feche e abra um novo CMD

---

### `composer install` falha com erro de extensões PHP

O PHP no Windows vem com extensões desabilitadas por padrão. Você precisa:
...
# Copie o arquivo de exemplo (geralmente em C:\php\)
copy C:\php\php.ini-development C:\php\php.ini
```

**2. Abrir o php.ini e habilitar as extensões:**
```bash
notepad C:\php\php.ini
```

Procure e remova o `;` do início de cada linha abaixo:
```ini
extension=openssl
extension=fileinfo
extension=zip
...
**3. Verificar se o php.ini está sendo carregado:**
```bash
php --ini
# Deve mostrar: Loaded Configuration File: C:\php\php.ini
```
...
O MySQL foi instalado com senha. Use `-p` para digitar a senha:

```bash
mysql -u root -p
# Vai solicitar a senha criada durante a instalação
```

---

### `php artisan` diz `Could not open input file`

Você está na pasta errada. Sempre entre na pasta do projeto antes:

```bash
cd C:\projetos\desafio-rootcode-api
php artisan serve
```

---

### `composer install` não encontra o `composer.json`

Mesmo problema — você precisa estar dentro da pasta do projeto:
...
Estes arquivos existem em **todo** projeto Laravel e não contêm lógica específica deste sistema:
...
> 💡 **Por que esses arquivos existem se não usamos?** Este projeto foi criado a partir de um starter kit Laravel que inclui o Fortify (autenticação avançada) e Inertia (frontend integrado). Optamos por usar nossa própria autenticação via Sanctum + API separada, então esses arquivos ficaram sem uso mas não prejudicam o funcionamento.